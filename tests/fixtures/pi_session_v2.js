// One Pi-session-owned child. The ordinary helper remains the busy fallback.
...
function createPiSession() {
  ...
  return {
    async compact(texts, tool) {
      if (stopped) return texts;
      if (tool !== "bash" || busy) return compactTexts(texts, tool);
      if (!texts.length || texts.some(t => typeof t !== "string")
        || texts.reduce((n,t) => n + Buffer.byteLength(t), 0) > LIMIT) return texts;
      busy = true;
      ...
      try {
        ...
        if (key === disabled) return await compactTexts(texts, tool);
        if (current && (current.key !== key || current.dead)) await retire(current);
        if (stopped) return texts;
        if (current?.dead) return await compactTexts(texts, tool);
...
export default function sift(pi) {
  ...
  pi.on("tool_result", async event => {
    ...
    const compacted = await (session ? session.compact(texts, event.toolName) : compactTexts(texts, event.toolName));
