...
This is a **development release-mode Sift v0.2.0 binary, not a shipped artifact**.
Final source/release qualification is pending. These measurements do not establish
Sift as a faster RTK replacement: on the seven non-control workloads its medians
...
| --- | --- | --- |
| Sift development binary | 0.2.0 | `fb9be54224069f898f8d88708af9930f389d782f89f6ba9783ce4dfbd8313eab` |
...
counted separately with the same warmed Sift tokenizer for every arm. All token
...
trips for the original stdout and stderr through a warmed Sift compact protocol,
including Python serialization/pipe overhead; it is not an extracted CPU time.

| Workload | Native tokens | Sift tokens | RTK tokens | Native ms | Sift ms | RTK ms | Sift warm ms |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
All ten native and Sift audits observed the requested command exactly once with
...
Sift timed stream matched the exact native original or its explicit-encoding
...
Sift's encoded log omitted the literal `Adjust validation for item 5` and its
encoded listing omitted the literal `module_16.txt`; both restored exactly. Every
other predeclared Sift marker was present, as were all native markers. An absent
...
latency, TTY behavior, agent turns or billing was measured. The same Sift tokenizer
...
conversion. Sift's runner bypasses streams below 256 bytes and can pass progress
